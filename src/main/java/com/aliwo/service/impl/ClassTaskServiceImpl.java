 ...
 *
 * @author:xuyy Date:2021/2/28 17:54
    ...
    public ServerResponse classScheduling(String semester) {
        try {
            ...
            if (StringUtils.isEmpty(semester)) {
                return ServerResponse.ofError("排课失败，没有相应的学期信息！");
            }
            ...
            Map<String, List<String>> individualMap = transformIndividual(resultGeneList, semester);
            // 5、遗传进化
            individualMap = geneticEvolution(individualMap, semester);
            // 6、分配教室并做教室冲突检测
            ...
            for (CoursePlan coursePlan : coursePlanList) {
                coursePlanDao.insertCoursePlan(coursePlan.getGradeNo(), coursePlan.getClassNo(),
                        coursePlan.getCourseNo(), coursePlan.getTeacherNo(), coursePlan.getClassRoomNo(),
                        coursePlan.getClassTime(), semester);
            }
            log.info("完成排课,耗时：" + (System.currentTimeMillis() - start) + "ms");
            return ServerResponse.ofSuccess("排课成功！");
        } catch (Exception e) {
            log.error("the error message is : " + "    " + e);
...
//            List<Classroom> classroomList1 = classRoomDao.selectByTeachbuildNoList(teachBuildNoList);
     ...
     * @return
     */
    private String chooseClassroom(int studentNum, String gene, List<ClassRoom> classroomList,
...
//        int temp = random.nextInt(max) % (max - min + 1) + min;
    ...
    private Boolean judgeClassroom(int studentNum, String gene, ClassRoom classroom, List<String> resultList) {
        ...
        if (courseAttr.equals(ConstantInfo.MAIN_COURSE) || courseAttr.equals(ConstantInfo.SECONDARY_COURSE)) {
            // 找到普通教室，普通教室的属性都是01
            if (classroom.getAttribute().equals("01")) {
                // 判断上课人数与教室容量
            ...
            // 剩余的课程应该要放在相对应的教室上课
            if (ClassUtil.cutGene(ConstantInfo.COURSE_ATTR, gene).equals(classroom.getAttribute())) {
                // 判断人数
                if (classroom.getCapacity() >= studentNum) {
                    // 判断该教室上课时间是否重复
                    return isFree(gene, resultList, classroom);
    ...
    private Boolean isFree(String gene, List<String> resultList, ClassRoom classroom) {
        ...
        if (resultList.size() == 0) {
            ...
            for (String resultGene : resultList) {
                ...
                if (ClassUtil.cutGene(ConstantInfo.CLASS_ROOM_NO, resultGene).equals(classroom.getClassRoomNo())) {
                    // 判断时间是否一样，一样则表示有冲突
     ...
     * @return
     */
    private Map<String, List<String>> geneticEvolution(Map<String, List<String>> individualMap, String semester) {
        // 遗传代数
        ...
        for (int i = 0; i < generation; ++i) {
            ...
            // 2、合拢所有班级的个体
            List<String> collectClass = collectGene(individualMap);
            // 2,3、变异
            resultGeneList = geneMutation(collectClass);
            // 4、冲突检测，消除冲突
            List<String> conflictList = conflictResolution(resultGeneList);
            // 5、将消除冲突后的个体再分班进入下一次进化
            individualMap = transformIndividual(conflictList, semester);
        }
    ...
    private List<String> replace(List<String> resuleGeneList, String oldGene, String newGene) {
        for (int i = 0; i < resuleGeneList.size(); i++) {
            if (resuleGeneList.get(i).equals(oldGene)) {
                ...
                log.info("===================执行替换方法================= 【】"
                        + "com.aliwo.service.impl.ClassTaskServiceImpl.replace");
...
//            String classroomNo = s.substring(26, 32);
    ...
    private List<String> selectGene(List<String> individualList) {
        ...
        do {
            ...
            if (firstIndex == secondIndex) {
                flag = false;
            } else if ("2".equals(ClassUtil.cutGene(ConstantInfo.IS_FIX, firstGene))
                    || "2".equals(ClassUtil.cutGene(ConstantInfo.IS_FIX, secondGene))) {
                // 上课时间已经固定
                flag = false;
            } else {
    ...
    private List<Map<String, List<String>>> coding(List<ClassTask> classTaskList) {
        ...
        for (ClassTask classTask : classTaskList) {
            ...
            if ("1".equals(classTask.getIsFix())) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    String gene = classTask.getIsFix() + classTask.getGradeNo() + classTask.getClassNo()
                            + classTask.getTeacherNo() + classTask.getCourseNo() + classTask.getCourseAttr() + ConstantInfo.DEAULT_CLASS_TIME;
            ...
            if ("2".equals(classTask.getIsFix())) {
                ...
                int size = classTask.getWeeksNumber() / 2;
                for (int i = 0; i < size; i++) {
                    // 如果是固定上课时间，那么上课时间不能为空
                    if (StringUtils.isEmpty(classTask.getClassTime())) {
                        return geneList;
                    }
                    // 获得设定的固定时间
     ...
     * @return
     */
    private Map<String, List<String>> transformIndividual(List<String> resultGeneList, String semester) {
        Map<String, List<String>> individualMap = new HashMap<>();
        // 查询开课的班级
        List<String> classNoList = classTaskDao.selectClassNo();
        if (classNoList != null && classNoList.size() > 0) {
            log.info("本学期【】：" + semester + "开课的班级有【】：" + classNoList.toString());
        }
    ...
    private List<CoursePlan> decoding(List<String> resultList) {
        ...
        for (String gene : resultList) {
            ...
            // 教室
            coursePlan.setClassRoomNo(ClassUtil.cutGene(ConstantInfo.CLASS_ROOM_NO, gene));
            // 上课时间
